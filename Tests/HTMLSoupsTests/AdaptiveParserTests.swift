/// This file contains tests for the AdaptiveParser class, which is responsible for
/// learning and adapting to different website structures. The tests verify the parser's
/// ability to learn from successful parses, maintain learned patterns, and handle
/// various content structures.
///
/// Key test areas:
/// - Basic content parsing
/// - Adaptive learning capabilities
/// - Learning persistence across instances
/// - Cross-article learning
/// - Configuration management
///
/// Dependencies:
/// - AdaptiveParser.swift: The main class being tested
/// - SelectorLearner.swift: For selector learning
/// - HTMLParser.swift: For content parsing
///
/// The tests focus on verifying the parser's ability to adapt to different
/// website structures and maintain learned patterns across multiple uses.
...
final class AdaptiveParserTests: XCTestCase {
    ...
    func testAdaptiveParsing() async throws {
        ...
        for url in urls {
            do {
                ...
                print(
                    "✅ Successfully parsed \(url.absoluteString.components(separatedBy: "://").last ?? url.absoluteString):"
                )
    ...
    func testLearningPersistence() async throws {
        ...
        XCTAssertEqual(
            firstConfig.titleSelector, secondConfig.titleSelector, "Title selectors should match")
        XCTAssertEqual(
            firstConfig.contentSelector, secondConfig.contentSelector,
            "Content selectors should match")
    }
    ...
    func testCrossArticleLearning() async throws {
        ...
        XCTAssertGreaterThan(
            firstArticle.content.count, 100, "First article content should be substantial")
        XCTAssertGreaterThan(
            secondArticle.content.count, 100, "Second article content should be substantial")
    }
}
