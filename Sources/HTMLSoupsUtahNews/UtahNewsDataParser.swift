        ...
        return try await parse(url: url) { [self] document in
            ...
            let contentBuilder = try document.select(config.contentSelector)
                .flatMap { article -> [Element] in
                    ...
                    let contentContainers = try article.select(
                        ".ArticleBody, .article-body, .story-body, .entry-content"
                    ).array()
            ...
            let cleanedContent =
                ...
                .replacingOccurrences(
                    of: "^Published\\s+on\\s+By\\s+", with: "", options: .regularExpression
                )
                .replacingOccurrences(
                    of: "^Posted\\s+on\\s+By\\s+", with: "", options: .regularExpression
                )
            ...
            if let authorSelector = config.authorSelector {
                author = try? self.extractText(from: document, selector: authorSelector)
                    ...
                    .replacingOccurrences(
                        of: "^Posted\\s+by\\s+", with: "", options: .regularExpression
                    )
                    .trimmingCharacters(in: .whitespacesAndNewlines)
            }
            ...
            if let dateString = try? self.extractText(
                from: document, selector: config.dateSelector ?? "")
            {
                ...
                for format in dateFormats {
                    ...
                    if let date = dateFormatter.date(
                        from: dateString.trimmingCharacters(in: .whitespacesAndNewlines))
                    {
                        publishedDate = date
                        break
                    }
            ...
            for selector in config.organizationSelectors {
                if let organizations = try? self.extractTextArray(
                    from: document, selector: selector)
                {
                ...
                if let locations = try? self.extractTextArray(from: document, selector: selector) {
                    for location in locations {
                        let cleanLocation = location.trimmingCharacters(in: .whitespacesAndNewlines)
                            .replacingOccurrences(
                                of: "^in\\s+", with: "", options: .regularExpression)
